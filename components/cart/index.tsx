...
export default function Cart() {
    ...
    const { orderDate, deliveryAddress, deliveryMethod } = useDeliveryMethodStore();

    // Functions
    const openCart = () => setIsOpen(true);
    const closeCart = () => setIsOpen(false);
    const getHref = (): { pathname: string } => {
        if (orderDate && deliveryMethod) {
            if (deliveryMethod === "pickup") {
                ...
                if (deliveryAddress) {
                    ...
                    return { pathname: "/delivery-method" };
                }
            }
        }
                            ...
                            >
                                <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
                                    ...
                                    <div className="mt-5 sm:mt-6 sm:grid sm:grid-flow-row-dense sm:grid-cols-3 sm:gap-3">
                                        <button
                                            type="button"
                                            className="inline-flex items-center w-full justify-center rounded-md border border-transparent bg-red-400 px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-red-700 focus:outline-none sm:text-sm"
                                            onClick={() => {
                                                clearCart();
                                                closeCart();
                                            }}
